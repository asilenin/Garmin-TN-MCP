...
| `get_activity_lactate(id)` | числовые отметки лактата из developer-поля TN Splits View: `[(время, ммоль, круг)]` |
...
Числовой лактат (новое): значения, записанные с часов полем **TN Splits View** (ConnectIQ),
лежат в **потоках** активности, не в комментарии. `get_activity_lactate` их читает — поле
матчится по `developerFieldNumber == 1` (индекс в потоке не стабилен, appID при sideload
нулевой / после публикации станет `7c294f6c-…`). Любой сэмпл > 0 — отметка; 0 = нет замера.
Каждая отметка привязывается к ближайшему кругу.
