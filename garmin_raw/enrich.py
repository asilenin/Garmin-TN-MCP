...
#                                   отвергает одиночный шип только с >=3 (INV-ROBUST-EXTREME — край
...
def _hr_recovery(series: list, laps: Optional[dict] = None) -> dict:
    ...
    последних 8с HR разрежён — расширение вместо одиночной точки, INV-ROBUST-EXTREME). Событие с
...
def _edge_hr(ts, hr_clean, end_ms, cap_s) -> tuple:
    ...
    окно с >= HR_RECOVERY_EDGE_MIN_SAMPLES сэмплов (медиана, НЕ одиночная точка — INV-ROBUST-EXTREME).
