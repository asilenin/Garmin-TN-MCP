...
Инвариант (QA 7.6 INV-NET-GUARD): read-тулы cache-only не ходят в сеть НИ НА КАКОМ пути; сетевые
...
(дрейф по транспорту). Двойной контур с непересекающимися слепыми зонами (INV-NET-GUARD-доп):
...
class ForbiddenNetworkAccess(BaseException):
    ...
    по типу, без инспекции traceback (traceback дрейфует от рефакторинга — INV-NET-GUARD разв. F)."""
